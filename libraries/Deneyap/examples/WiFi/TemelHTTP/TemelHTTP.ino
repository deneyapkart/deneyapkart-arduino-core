...
void setup() {
    ...
    if((wifiMulti.run() == WL_CONNECTED)) {
        ...
        if(httpCode > 0) {
            // HTTP başlığı gönderildi ve Sunucu yanıt başlığı işlendi
            USE_SERIAL.printf("[HTTP] Erişin... KOD: %d\n", httpCode);
            ...
            USE_SERIAL.printf("[HTTP] Erişim... hatası, Hata: %s\n", http.errorToString(httpCode).c_str());
        }
        http.end();
    } 
}
